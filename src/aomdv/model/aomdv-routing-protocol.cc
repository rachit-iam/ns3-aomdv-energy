                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  NS_LOG_UNCOND("check route input");
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (!m_routingTable.LookupRoute (origin, toOrigin))
    ...
    {
      //NS_LOG_UNCOND("check bululu");
      if (toOrigin.GetValidSeqNo ())
        {
          //NS_LOG_UNCOND("check bululu2");
          ...
          //NS_LOG_UNCOND("FAK");
          toOrigin.PathInsert(dev, src, rreqHeader.GetHopCount ()+1, 
                           Time ((2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime)), 
                           rreqHeader.GetFirstHop (), 
                           m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0));
        }
                            ...
                            Ipv4Address firstHop)
{
  ...
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (12);// TODO toOrigin.GetAdvertisedHopCount ()+1 CHANGE BAD
  packet->AddPacketTag (tag);
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      if ((toDst.GetNumberofPaths() == 0) || (int32_t (rrepHeader.GetDstSeqno ()) - int32_t (toDst.GetSeqNo ())) > 0)
        {
      ...
      // (iii) the sequence numbers are the same, but the route is marked as inactive.
               ...
               && (toDst.GetAdvertisedHopCount () > rrepHeader.GetHopCount ()))
        {
	      ...
	      // CHANGE
              ...
              else if (forwardPath->GetHopCount () == hop)
                {
  ...
  if (tag.GetTtl () < 2)
    {
      //NS_LOG_UNCOND ("TTL exceeded. Drop RREP destination " << dst << " origin " << rrepHeader.GetOrigin ());
