  ...
  if (int32_t (rreqHeader.GetOriginSeqno ()) - int32_t (toOrigin.GetSeqNo ()) > 0)
    {
      ...
      reversePath = toOrigin.PathInsert (
          ...
          m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0), rreqHeader.GetMRE());
                            ...
                            Ipv4Address firstHop)
{
   ...
   */
  if (rreqHeader.GetUnknownSeqno() || m_seqNo < rreqHeader.GetDstSeqno())
    m_seqNo++;
  ...
  RrepHeader rrepHeader ( /*prefixSize=*/ 0, /*hops=*/ 0, /*dst=*/ rreqHeader.GetDst (),
                          ...
                          /*lifeTime=*/ m_myRouteTimeout, /*MRE*/ INFINITY3);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.PathFind()->GetNextHop (), AOMDV_PORT));//TODO SENDING TO ONLY ONE PATH
}
  ...
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor) || (toNeighbor.GetNumberofPaths() == 0))
    {
      ...
      newEntry.PathInsert (/*device=*/ dev, /*nextHop=*/ rrepHeader.GetDst (), /*hop=*/ 1, 
                           ...
                           /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0), INFINITY3);
      m_routingTable.AddRoute (newEntry);
    }
  else
    {
